...
model = ViTModel().cuda()
state_dict = torch.load("save89.pt")
...
def estimate_random_pose(mkpts0, mkpts1, feats0, feats1):
    calc_pose = model(mkpts0, mkpts1, feats0, feats1)
    R = quat_to_rot(calc_pose[:, :4])
    t = calc_pose[:, 4:]
    return (R, t[:, 0], mask.ravel() > 0)
...
def compute_pose_errors(data, config):
    ...
    feats0 = data["feats_trans0"]
    feats1 = data["feats_trans1"]
        ...
        if config.LOFTR.EVAL_TIMES >= 1:
            ...
            for _ in range(5):
                ...
                if _ >= config.LOFTR.EVAL_TIMES:
                    continue
                # bpts0 = bpts0[shuffling][:512, :]
                # bpts1 = bpts1[shuffling][:512, :]
