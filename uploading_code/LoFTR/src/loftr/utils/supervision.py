...
def spvs_fine(data, config, logger = None):
    ...
    if m == 0:  # special case: there is no coarse gt
        ...
        if m_ids.numel() == 0:  # special case: there is no pixel-level gt
            ...
            data.update({'expec_f': torch.zeros(1, 2, device=device)})
            data.update({'expec_f_gt': torch.zeros(1, 2, device=device)})
            data.update({"expec0_f_gt": torch.zeros(1, 2, device=device)})
            data.update({"expec1_f_gt":  torch.zeros(1, 2, device=device)})
            data.update({"expec0_f": torch.zeros(1, 2, device=device)})
            data.update({"expec1_f":  torch.zeros(1, 2, device=device)})
        else:
            expec_f_gt = expec_f_gt[m_ids, i_ids]
            expec0_f, expec1_f = data['expec0_f'], data['expec1_f']
            
            del data['sim_matrix_ff']
            if 'sim_matrix_ff_new' in data: 
                del data['sim_matrix_ff_new']
            '''
            ...
            sim_matrix_f = sim_matrix_f.reshape(-1, WW, W+2, W+2) # [M, WW, W+2, W+2]
            sim_matrix_f = sim_matrix_f[m_ids, i_ids, j_ids_di, j_ids_dj]
            sim_matrix_f = sim_matrix_f.reshape(-1, 9)

            sim_matrix_f = F.softmax(sim_matrix_f / 10.0, dim=-1)
            ...
            '''
            '''
            delta_new = create_meshgrid(3, 3, True, sim_matrix_f_new.device).to(torch.long) # [1, 3, 3, 2]
            # Note that j_ids_di & j_ids_dj in (i, j) format while delta in (x, y) format
            j_ids_di = j_ids_di[...,None,None].expand(-1, 3, 3) + delta[None, ..., 1]
            j_ids_dj = j_ids_dj[...,None,None].expand(-1, 3, 3) + delta[None, ..., 0]
            ...
            expec1_f = expec1_f + (coords_normalized * (3 // 2) * scale1)
            '''
            _, expec0_f_gt = warp_kpts(expec0_f.reshape(N,-1,2), data['depth0'], data['depth1'], data['T_0to1'], data['K0'], data['K1'])
            expec0_f_gt = expec0_f_gt - expec0_f_gt.round().long()
            expec0_f = expec0_f - expec0_f.round().long()
            _, expec1_f_gt = warp_kpts(expec1_f.reshape(N,-1,2), data['depth1'], data['depth0'], data['T_1to0'], data['K1'], data['K0'])
            expec1_f_gt = expec1_f_gt - expec1_f_gt.round().long()
            expec1_f = expec1_f - expec1_f.round().long()
            data.update({"expec_f_gt": expec_f_gt.reshape(-1, 2)})
            data.update({"expec0_f_gt": expec0_f_gt.reshape(-1, 2)})
            data.update({"expec1_f_gt": expec1_f_gt.reshape(-1, 2)})
            data.update({"expec0_f": expec0_f, 
                            "expec1_f": expec1_f
            })
