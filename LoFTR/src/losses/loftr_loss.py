...
class LoFTRLoss(nn.Module):
    ...
    def _compute_local_loss_l2(self, expec_f, expec_f_gt):
        ...
        if correct_mask.sum() == 0:
            if self.training:  # this seldomly happen when training, since we pad prediction with gt
                ...
                return None
        # print("correct mask shape: ", correct_mask.shape, expec_f.shape, expec_f_gt.shape)
    ...
    def _compute_cross_loss(self, mkpts1_f, mkpts0_f_new, expec0_f_gt, expec1_f_gt):
        correct_mask0 = torch.linalg.norm(expec0_f_gt, ord=float('inf'), dim=1) < self.correct_thr
        correct_mask1 = torch.linalg.norm(expec1_f_gt, ord=float('inf'), dim=1) < self.correct_thr
        if mkpts1_f.shape[0] != mkpts0_f_new.shape[0]:
            # correct_mask0 = torch.zeros_like(correct_mask0, dtype=torch.bool)
            # correct_mask1 = torch.zeros_like(correct_mask1, dtype=torch.bool)
            correct_mask0 = [0]
            correct_mask1 = [0]            
            print("should be here?")
        else:
            correct_mask0 = torch.bitwise_and(correct_mask0, correct_mask1)
            correct_mask1 = correct_mask0
            if correct_mask0.sum() == 0:
                if self.training:  # this seldomly happen when training, since we pad prediction with gt
                    logger.warning("assign a false supervision to avoid ddp deadlock")
                    correct_mask0[0] = True
                else:
                    return None
            if correct_mask1.sum() == 0:
                if self.training:  # this seldomly happen when training, since we pad prediction with gt
                    logger.warning("assign a false supervision to avoid ddp deadlock")
                    correct_mask1[0] = True
                else:
                    return None
        # min_length = min(mkpts1_f.shape[0], mkpts0_f_new.shape[0])
        # print("their shape: ", mkpts1_f.shape, mkpts0_f_new.shape)
        # print("their shape: ", mkpts1_f.shape, mkpts0_f_new.shape, correct_mask.shape)
        # loss = ((mkpts1_f[correct_mask0] - mkpts0_f_new[correct_mask1]) ** 2).sum(-1)
        loss = ((mkpts1_f[correct_mask0] - mkpts0_f_new[correct_mask1]) ** 2).sum(-1)
        return loss.mean()
    ...
    def forward(self, data):
        ...
        if 'expec_f' not in data:
            ...
            sim_matrix_f_new = data['sim_matrix_ff_new']
            del data['sim_matrix_ff'], data['m_ids_f'], data['i_ids_f'], data['j_ids_f_di'], data['j_ids_f_dj'], data['sim_matrix_ff_new']
            ...
            print("coords_normalized: ", coords_normalized.shape)
            data.update({'expec_f': coords_normalized})
        '''
        # import pdb; pdb.set_trace()
        # loss_l = self._compute_local_loss_l2(data['expec_f'], data['expec_f_gt'])
        loss_l = self._compute_local_loss_l2(data['expec0_f'], data['expec0_f_gt'])
        loss_l += self._compute_local_loss_l2(data['expec1_f'], data['expec1_f_gt'])
        loss += loss_l * self.loss_config['local_weight']
        '''
        if 'mkpts0_f_new' in data: 
            loss_cross = self._compute_cross_loss(data['mkpts1_f'], data['mkpts0_f_new'])
            loss += loss_cross * 0.0001
            # print("yes, cross loss added")
        '''
        if 'expec1_f_new' in data:
            # import pdb; pdb.set_trace()
            loss_cross = self._compute_cross_loss(data['expec1_f_new'], data['expec1_f'], data['expec0_f_gt'], data['expec1_f_gt'])
            loss += loss_cross * 1e-5
            # print("yes, cross loss added if: ", loss_cross)
            print("their shape: ", data['expec1_f'].shape, data['expec1_f_new'].shape, data['expec0_f_gt'].shape, data['expec1_f_gt'].shape, loss_cross)
