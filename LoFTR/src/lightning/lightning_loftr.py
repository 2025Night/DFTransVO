    ...
    def optimizer_step(
        ...
        for name, param in self.matcher.named_parameters():
            if param.grad is None:
                print(f"Rank {torch.distributed.get_rank()}, Grad is None for {name}") 
        print(f"Rank {torch.distributed.get_rank()}, Batch is {self.global_step}")
        '''
        optimizer.zero_grad()
    
    def _trainval_inference(self, batch):
        # print(f"Rank {torch.distributed.get_rank()}, Input shape: {batch['image0'].shape}, {batch['image1'].shape}, {batch['image2'].shape}")
